...
class Exporter(object):
    ...
    def s3_export(data):
        ...
        with gzip.open('mapsforchange.js.gz', 'wb') as f:
            f.write(str(script_content).encode('utf-8'))
            
        with open('mapsforchange.json', 'w') as f:
            f.write(raw_data)
        ...
        key = bucket.get_key('output/mapsforchange.js.gz')
        key_raw = bucket.get_key('raw/mapsforchange.json')
        
        if key is None: 
            print("Creating New Bucket")
            key = bucket.new_key('output/mapsforchange.js.gz')
            
        if key_raw is None:
            print("Creating New Raw File")
            key_raw = bucket.new_key('raw/mapsforchange.json')
        
        # Upload data to S3
        print("Uploading RAW to S3")
        key_raw.set_contents_from_filename('mapsforchange.json')
        key_raw.set_acl('public-read')
        
        print("Uploading GZIP to S3")
        key.set_metadata('Content-Type', 'text/plain')
        key.set_metadata('Content-Encoding', 'gzip')
        key.set_contents_from_filename('mapsforchange.js.gz')
        ...
        #Delete all files
        os.remove("mapsforchange.js.gz")
        os.remove("mapsforchange.json")
